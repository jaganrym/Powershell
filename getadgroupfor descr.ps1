...
foreach($group in $groups)
{
    ...
    if($grps)
    {
        ...
        select @{l='domain';e={$($domain)}},Name,SamAccountName, groupcategory, groupscope, CanonicalName, extensionAttribute15,whenChanged, whenCreated, @{name='MembersCount';expression={$_.members.count}}
